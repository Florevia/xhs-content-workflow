...
- 本文件（`AGENTS.md`）是规则唯一权威来源；`.cursor/rules/xhs-workflow.mdc` 必须与本文件保持内容一致（尤其是第 8 节业务规则和错误修复记录表）。
...
8. **错误自我修复规则**：每次流程执行过程中出现报错、失败或产出不符合规则的情况，必须按以下步骤处理，不得跳过：
   ...
   - **总结沉淀**：将本次错误的「触发场景、根本原因、修复方案」提炼为一条通用规则或注意事项，追加写入本文件的「## 错误修复记录」章节，并同步写入 `.cursor/rules/xhs-workflow.mdc`。禁止只在对话中口头总结而不落盘。
   - **规则升级**：如果错误反映出现有规则（第 1-7 条或已有的错误修复记录）本身有遗漏或表述不清，应直接修改对应条目，而不是仅新增一条记录，避免规则冗余、互相矛盾。
   - **验证**：修复并更新规则后，重新执行一次相关步骤（或等效的最小验证），确认问题已解决，再继续后续流程。

## 错误修复记录

> 每次流程出错并修复后，请在下方追加一条记录，格式：`日期 | 触发场景 | 根本原因 | 修复方案/新增规则`。此表随项目推进持续积累，是所有 Agent 排查同类问题时的首要参考。

| 日期 | 触发场景 | 根本原因 | 修复方案/新增规则 |
| --- | --- | --- | --- |
| 2026-07-08 | 重新生成并发布"港股打新中签机制"笔记时，`auto_publish.py` 首次调用生图报错 `RuntimeError: ChatGPT returned 0 images, expected 5`；随后手动直接运行 `chatgpt_automation.py` 用同一批提示词却一次成功生成全部图片 | ChatGPT 网页自动化偶发瞬时故障（可能是新开标签页/页面渲染较慢导致首次检测不到回复中的图片），并非提示词或代码逻辑错误；`images.py::generate_images` 之前没有重试机制，单次瞬时失败会直接中断整条发布流程 | 在 `src/xhs_workflow/images.py::generate_images` 增加默认重试一次的逻辑（`DEFAULT_GENERATE_RETRIES = 1`），首次 ChatGPT 批次生成失败时自动重跑一次子进程调用，仍失败才向上抛出异常；已在 `tests/test_core.py::ImageAutomationTests` 补充重试成功与重试耗尽两个用例并跑通全部单测 |
| 2026-07-08 | 已发布的"港股打新中签机制"笔记收到小红书平台金融内容合规提醒（要求不得提供无资质个股分析/收益承诺、不得冒充专业人士、不得推广非法集资传销、不得发布违规借贷/高利贷信息），需要编辑修订 | 现有 `docs/compliance_rules.md` 只笼统要求"金融内容加免责声明"，未针对个股分析、收益承诺、专业人士包装、非法集资/传销、违规借贷等具体金融红线给出可执行的检查项，导致生成/复核时容易遗漏；同时项目 `xhs-publish` CLI（`cli.py`）只支持新建笔记的填写/发布/存草稿流程，没有"笔记管理→编辑已发布笔记"的子命令，无法在不重新发布的前提下自动修订已发布内容 | 1) 在 `docs/compliance_rules.md` 新增第 11 条，明确金融内容的具体红线清单（个股分析/收益承诺/专业人士冒充/非法集资传销/违规借贷高利贷/杠杆操作细节），并要求适用于笔记、评论、用户资料等所有发布场景；2) 已发布笔记的合规修订需要人工在小红书创作者后台「笔记管理」中手动编辑保存，本项目当前 CLI 不具备该能力，禁止用项目外的浏览器自动化（如通用 MCP 工具）代替登录操作已发布笔记，如确需自动化编辑已发布笔记，须先向仓库所有者提出并显式扩展 `cli.py` 增加对应子命令，而不是绕开唯一执行方式 |
